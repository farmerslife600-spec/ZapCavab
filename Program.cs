...
// ZapCavab — giriş nöqtəsi.
// Layihə indi WPF layihəsidir (bax: ZapCavab.csproj — OutputType=WinExe, UseWPF=true,
// net8.0-windows). Pəncərə hələ qurulmayıb, ona görə köhnə konsol test dəstini SİLMƏDİK —
// aşağıdakı TestleriIslet() metoduna köçürdük ki, lazım olanda yenə işə salına bilsin.

// WinExe (WPF) tətbiqlərində konsol çıxışının kodlaşdırması default olaraq UTF-8 deyil —
// bu, Azərbaycan (ə,ş,ç,ğ,ö,ü,ı) və kiril hərflərinin "?" kimi görünməsinə səbəb olur.
// Açıq şəkildə UTF-8 təyin edirik ki, konsol testləri düzgün oxuna bilsin.
// Qeyd: çıxış başqa proqrama yönləndirilibsə (piped) bu təyinat xəta verə bilər —
// belə halda sadəcə keçirik, bu yalnız görünüş məsələsidir, funksionallığa təsir etmir.
try
{
...
// Bütün köhnə konsol testlərini (axtarış + CRUD + Excel idxal) ardıcıl işə salır.
// Baza qurma, backup, seed doldurma və bütün test halları buradadır — məzmun eyni qalıb,
// sadəcə top-level koddan bu adlı metoda köçürülüb.
static void TestleriIslet()
{
    ...
    // 8) Excel idxal testi — CRUD testləri kimi AYRICA müvəqqəti bazada işləyir.
    // Əvvəllər əsl bazaya (db) idxal edirdi — bu, axtarış testlərinin nəticə sayını
    // (məs. "3 gözlənilən") sonrakı işə salınmalarda pozurdu, çünki idxal olunan mallar
    // əsl bazada qalıb qalırdı. İndi tamamilə təcrid olunmuş bazadadır, əsl bazaya toxunmur.
    Console.WriteLine("\n=== Excel idxal testi ===\n");

    var numuneFayl = Path.Combine(AppContext.BaseDirectory, "numune_mallar.xlsx");
    NumuneExcelYarat(numuneFayl);

    var excelTestBazaYolu = Path.Combine(AppContext.BaseDirectory, "zapcavab_excel_test.db");
    if (File.Exists(excelTestBazaYolu))
        File.Delete(excelTestBazaYolu);

    using (var excelTestDb = new AppDbContext(excelTestBazaYolu))
    {
        excelTestDb.Database.EnsureCreated();
        excelTestDb.IlkVeriləriYarat(); // eyni 7 seed mal — idxal faylındakı 2 "yenilənmə" sətri bunlarla uyğun gəlsin deyə

        var importServisi = new ExcelImportService(new PartService(excelTestDb));

        Console.WriteLine($"Bazada idxaldan ƏVVƏL: {excelTestDb.Parts.Count()} mal\n");
        ...
        Console.WriteLine($"\nBazada idxaldan SONRA: {excelTestDb.Parts.Count()} mal");
    }
    ...
    {
        // Problem deyil — növbəti işə salınmada yenidən silinməyə cəhd olunacaq.
    }
